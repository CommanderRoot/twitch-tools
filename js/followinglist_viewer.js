		...
		success: function(result, textStatus, jqXHR) {
			...
			if(typeof result['user_id'] !== 'undefined' && typeof result['expires_in'] !== 'undefined') {
				...
				if(result['expires_in'] >= 8*60*60 || result['expires_in'] === 0) {
					...
					if(urlParams.get('username') !== null && urlParams.get('username') !== '') {
						$('#lookup-user-username').val(urlParams.get('username'));
		...
		success: function(result, textStatus, jqXHR) {
			...
			if(typeof result['data'] !== 'undefined' && typeof result['data'][0] !== 'undefined') {
				...
				if(typeof optOutChannelIDs[localUser.id] === 'boolean' && optOutChannelIDs[localUser.id] === true) {
					$('#status').html('<div class="alert alert-warning" role="alert">This user has asked its follow list to be not listed with this tool.</div>');
					$('#lookup-user-button').removeAttr('disabled');
				} else {
					$(document).attr('title', $(document).attr('title') + ' - ' + result['data'][0]['login']);
					$('#status').html('<div class="alert alert-info" role="alert">Loading follows ... <span id="follower-loading-status"></span> (<a href="#" id="abort-loading-button">abort</a>)</div>');
					$('#content').html('');
					getFollowersFromAPI('');
				...
				$('#status').html('<div class="alert alert-warning" role="alert">No user with this name was found!</div>');
				$('#lookup-user-button').removeAttr('disabled');
			}
		},
		error: function(jqXHR, textStatus, errorThrown) {
			const err = jqXHR.status + ', ' + textStatus + ', ' + errorThrown;
			$('#status').html('<div class="alert alert-warning" role="alert">Error while getting user info, please try again. (' + escapeHtml(err) +')</div>');
			$('#lookup-user-button').removeAttr('disabled');
		},
	});
}
...
function getFollowersFromAPI(cursor) {
	...
	const requestStartTime = new Date();
	$.ajax({
		url: TwitchAPI.url + '/helix/users/follows?first=100&from_id=' + encodeURIComponent(localUser.id) + (cursor.length > 0 ? ('&after=' + encodeURIComponent(cursor)) : ''),
		type: 'GET',
		timeout: 30000,
		cache: false,
		headers: {
		...
		success: function(result, textStatus, jqXHR) {
			...
			if(typeof result['data'] !== 'undefined') {
				...
				if(typeof result['pagination'] !== 'undefined' && typeof result['pagination']['cursor'] !== 'undefined' && result['pagination']['cursor'].length > 0 && stopLoading === false) {
					...
					$('#status').html('<div class="alert alert-success" role="alert">Loading of ' + escapeHtml(new Intl.NumberFormat().format(followerList.length)) + ' follows done!</div>');
					renderFollowerList();
				...
				$('#status').html('<div class="alert alert-warning" role="alert">Error while getting follows, retrying... (' + JSON.stringify(result) +')</div>');
				// Retry request
		...
		error: function(jqXHR, textStatus, errorThrown) {
			...
			if(jqXHR.status == 429) {
				...
				$('#status').html('<div class="alert alert-warning" role="alert">Error while getting follows, retrying... (' + escapeHtml(err) +')</div>');
				getFollowersFromAPI(cursor);
			}
		},
	});
...
function renderFollowerList(filter) {
	...
	if(followerList.length == 0) {
		$('#content').text('No follows found on account: ' + localUser.login);
	...
	$(followerList).each(function(i, e) {
		...
		html += '<tr class="follower-card' + knownBot + disabledAccount + '" data-userid="' + escapeHtml(e.userID) + '">';
			...
			html += '<td>';
				html += '<a href="https://www.twitch.tv/popout/' + escapeHtml(e.userName) + '/viewercard/' + escapeHtml(localUser.login) + '?popout=" target="_blank" rel="noopener">Mod card followed</a> / ';
				html += '<a href="https://www.twitch.tv/popout/' + escapeHtml(localUser.login) + '/viewercard/' + escapeHtml(e.userName) + '?popout=" target="_blank" rel="noopener">Mod card local</a> / ';
	...
	if(filteredResultCounter >= filteredResultCounterMax) {
		...
		html = '<div class="row mb-4" id="results"><p class="pl-4">No follows found using this filter.</p></div>';
	}
...
function renderTopDayStats() {
	...
	$.each(followsPerDaySort.sort(function(a, b) { return b.follows - a.follows; }), function(i, e) {
		...
		if(i == 0) {
			html += '<div id="topDays"><div class="row mt-4"><p class="pl-4 col-md-auto"><b>Total follows:</b> ' + escapeHtml(new Intl.NumberFormat().format(followerList.length))  + '</p><p class="pl-4 col-md-auto"><b>Filtered follows:</b> <span id="filteredFollowsText"></span></p></div><div class="row"><p class="pl-4 col-md-auto"><b>Days with the most follows:</b><br>';
		}
...
function exportFollowerListAsCSV() {
	const exportFilename = 'followinglist_' + localUser.login + '__' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.csv';
...
function exportFilteredFollowerListAsCSV() {
	const exportFilename = 'followinglist_filtered_' + localUser.login + '__' + moment().format('YYYY-MM-DD_HH-mm-ss') + '.csv';
	let csv = 'userName,userID,accCreatedAt,followCreatedAt,broadcasterType,accountType,defaultLogo,isKnownBot' + "\r\n";
	let userIds = {};
...
function showChannelSelect() {
	let html = '<div class="row pl-4">';
		html += '<div id="channel-select">';
			...
			html += '<div class="pt-2">';
			html += '<button type="button" class="btn btn-primary btn-sm" id="lookup-user-button">Show follows</button>';
			html += '</div>';
		html += '</div>';
	html += '</div>';
	$('#content').html(html);
	...
	$('#lookup-user-button').on('click', function(e) {
		...
		if($('#lookup-user-username').val().trim().length == 0) {
			$('#status').html('<div class="alert alert-warning" role="alert">Please enter a username</div>');
			return;
		}
		...
		url.searchParams.set('username', $('#lookup-user-username').val());
...
$(function() {
	...
	window.addEventListener('beforeunload', (event) => {
		if(getFollowersRequestsDone === false && followerList.length >= 20000) {
			event.returnValue = 'The follow list is still loading, are you sure you want to leave?';
		}
	});
});
